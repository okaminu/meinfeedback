...
class MainWidgetDirector implements WidgetDirectorInterface
{
    ...
    public function build(
        ...
        $baseImage = new ImageBaseElement($this->resources());
        $baseImage->setBackgroundColor($backgroundColor);
        $this->builder->addElement($baseImage);

        $repText = new ImageRepeatTextElement($this->resources());
        ...
        $text = new ImageTextElement($this->resources());
        $text->setText($date)
            ->setPositionX(120)
            ->setPositionY(214)
            ->setFontColorCode($textColor);
        ...
        $comment = new ImageCommentElement($this->resources());
